...
export const useNotificationCustomerSocket = () => {
  ...
  useEffect(() => {
    ...
    connectSocket();
    socket.emit('join_customer_room', user.userId);

    const handleNewNotification = (data: Notification) => {
      queryClient.setQueryData(
        ['customer-notifications', user.userId],
      ...
      queryClient.setQueryData(
        ['customer-unread-count', user.userId],
        (old: { total: number }) => ({
          total: (old?.total ?? 0) + 1,
        }),
      );
